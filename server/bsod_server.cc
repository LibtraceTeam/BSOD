...
{
    printf("Usage: %s [-h] [-s <source>] [-p <listenport>] [-f <filter>] [-c <colour module] [-l <left position module] [-r <right position module>]\n", name);
    exit(0);
}
...
int main(int argc, char *argv[])
{
    ...
    // rt stuff
    static struct libtrace_t *trace = 0;
    ...
    */
    ...
    while( (opt = getopt(argc, argv, "hs:p:f:c:l:r:")) != -1)
    {
	    switch(opt)
	    {
		    case 'h':
    ...
    modptrs.colour = (colfptr) dlsym(colourhandle, "get_colour");
    ...
    assert(lefthandle);
    modptrs.left = (posfptr) dlsym(lefthandle, "get_position");
    if ((error = dlerror()) != NULL) {
	    printf("%s\n",error);
	    assert(modptrs.left);
    }
    ...
    assert(righthandle);
    modptrs.right = (posfptr) dlsym(righthandle,"get_position");
    if ((error = dlerror()) != NULL) {
	    printf("%s\n",error);
	...
	for(;;) 
	{
	    ...
	    if(per_packet(erfptr, psize, ts, &modptrs) != 0)
		break;
